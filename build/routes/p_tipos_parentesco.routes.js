...
var _p_tipos_parentesco = _interopRequireDefault(require("../controllers/p_tipos_parentesco.controller"));
...
const _p_tipos_parentescoController = new _p_tipos_parentesco.default(_index.default.p_tipos_parentesco);
...
router.get('/consultar', _auth.default.verificarGestor, async (req, res, next) => {
  if (_underscore.default.isEmpty(req.query)) {
    _p_tipos_parentescoController.obtenerTodos(req, res, next);
  } else if (req.query.id) {
    _p_tipos_parentescoController.obtenerPorId(req, res, next);
  } else {
    res.status(404).json({
