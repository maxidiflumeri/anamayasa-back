...
var _default = {
  generaArchivo: async queryParams => {
    ...
    let queryCorreos = `SELECT d.id_deudor, c.correo, c.id_tipo_correo , pc.descripcion
                            ...
                            INNER JOIN p_tipos_correo as pc on c.id_tipo_correo = pc.id_tipo_correo
                            ${queryWhere}`;
    let queryRedes = `SELECT d.id_deudor, rs.usuario, rs.id_red_social , prs.descripcion
                            FROM deudores as d
                            INNER JOIN redes_sociales as rs on d.id_deudor = rs.id_deudor
                            INNER JOIN p_redes_sociales as prs on rs.id_red_social = prs.id_red_social
                            ${queryWhere}`;
    let queryDirecciones = `SELECT d.id_deudor, di.id_tipo_direccion, td.descripcion, di.calle, di.numero, di.piso, di. departamento, 
                                di.cod_postal, di.localidad, di.provincia, di.efectivo
                                FROM deudores as d
                                INNER JOIN direcciones as di on d.id_deudor = di.id_deudor
                                INNER JOIN p_tipos_direccion as td on di.id_tipo_direccion = td.id_tipo_direccion
                                ${queryWhere}`;
    let queryVinculos = `SELECT d.id_deudor,tp.descripcion, v.nombre, v.telefono, v.correo
                            FROM deudores as d
                            INNER JOIN vinculos as v on v.id_deudor = d.id_deudor
                            INNER JOIN p_tipos_parentesco as tp on tp.id_tipo_parentesco = v.id_tipo_parentesco
                            ${queryWhere}`;
    let queryFacturas = `SELECT d.id_deudor, f.id_factura, f.nro_factura, f.id_convenio, f.fecha_comprobante, f.fecha_vencimiento, f.cant_cartas, f.id_correo,
                            f.clave_pago, f.importe1, f.importe2, f.importe3, f.importe4, f.importe5,f.aux1
                            FROM deudores as d
                            INNER JOIN facturas as f on d.id_deudor = f.id_deudor
                            ${queryWhere}`;
    let queryPagos = `SELECT d.id_deudor, p.id_pago, p.nro_comprobante, p.id_entidad, p.id_tipo_pago , p.fecha_pago, p.fecha_aplicacion,
                        p.fecha_rendicion, p.importe_total, p.importe_aplicado, p.comentario, p.fecha_registro, p.anulado, p.aux1
                        FROM deudores as d
                        INNER JOIN pagos as p on d.id_deudor = p.id_deudor
                        ${queryWhere}`;
    let queryComentarios = `SELECT d.id_deudor, c.id_comentario, c.id_usuario,u.nombre_completo, c.comentario, c.fecha
                                FROM deudores as d
    ...
    let queryConvenios = `SELECT d.id_deudor, c.id_convenio, c.fecha, c.cuotas, c.id_tipo, c.importe, c.id_tasa, c.id_direccion,
                            ...
                            FROM deudores as d
                            INNER JOIN convenios as c on d.id_deudor = c.id_deudor
                            ${queryWhere}`;
    return new Promise(async (resolve, reject) => {
      ...
      try {
        let deudores = await _connection.default.query(queryDeudores);
        let telefonos = await _connection.default.query(queryTelefonos);
        let correos = await _connection.default.query(queryCorreos);
        let redes = await _connection.default.query(queryRedes);
        let direcciones = await _connection.default.query(queryDirecciones);
        let vinculos = await _connection.default.query(queryVinculos);
        let facturas = await _connection.default.query(queryFacturas);
        let pagos = await _connection.default.query(queryPagos);
        let comentarios = await _connection.default.query(queryComentarios);
        let convenios = await _connection.default.query(queryConvenios);
        response.push(deudores[0]);
        response.push(telefonos[0]);
        response.push(correos[0]);
        response.push(redes[0]);
        response.push(direcciones[0]);
        response.push(vinculos[0]);
        response.push(facturas[0]);
        response.push(pagos[0]);
        response.push(comentarios[0]);
        response.push(convenios[0]);

        _exportXls.default.exportarExcel(sheetNames, response, `./files/${id_empresa}_${remesa_desde}_${remesa_hasta}.xlsx`);
