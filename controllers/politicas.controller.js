class politicasController {
    ...
    constructor(model) {
      ...
      async obtenerPorId(req, res, next){
        try{
            const response = await this._model.findAll({where: {id_politica: req.query.id}})
            res.status(200).json(response)
        }catch(error){              
            res.status(500).json({
                mensaje: 'Ocurrio un error'
      ...
      async actualizar(req, res, next){
        try{
            const response = await this._model.update(req.body, {where: {id_politica: req.params.id}})
            res.status(200).json(response)
      ...
      async eliminar(req, res, next){
        try{
            const seBorro = await this._model.destroy({where: {id_politica: req.params.id}})
            if(seBorro){
