...
class redes_socialesController {
    ...
    async agregar(req, res, next) {
        try {
            ...
            serviceTransaccion.generaTransaccion(req.headers.token, req.body.id_deudor, 31, `Se agrega usuario ${req.body.usuario}`, null, null)
            res.status(200).json(response)
        } catch (error) {
            res.status(500).json({
                mensaje: 'Ocurrio un error'
    ...
    async actualizar(req, res, next) {
        try {
            ...
            if(usuarioRed[0].usuario != req.body.usuario){
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 33, `Se modifica usuario ${req.params.usuario} por el usuario ${req.body.usuario}`, null, null)
            }
            if(usuarioRed[0].id_red_social != req.body.id_red_social){
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 33, `Se modifica tipo red ${usuarioRed[0].id_red_social} por el tipo red ${req.body.id_red_social}`, null, null)
            }
            res.status(200).json(response)
    ...
    async eliminar(req, res, next) {
        try {
            ...
            if (seBorro) {
                ...
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 32, `Se elimina usuario ${req.params.usuario}`, null, null)
            } else {
