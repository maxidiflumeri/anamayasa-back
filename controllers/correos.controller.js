...
class correosController {
    ...
    async agregar(req, res, next) {
        try {
            ...
            serviceTransaccion.generaTransaccion(req.headers.token, req.body.id_deudor, 23, `Se agrega correo ${req.body.correo}`, null, null)
            res.status(200).json(response)
        } catch (error) {
            res.status(500).json({
                mensaje: 'Ocurrio un error'
    ...
    async actualizar(req, res, next) {
        try {
            ...
            if (correo[0].correo != req.body.correo) {
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 28, `Se modifica correo ${req.params.correo} por el correo ${req.body.correo}`, null, null)
            }
            if (correo[0].efectivo != req.body.efectivo) {
                if (req.body.efectivo == 1) {
                    serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 28, `Se marca como principal el correo ${req.body.correo}`, null, null)
                } else {
                    serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 28, `Se desmarca como principal el correo ${req.body.correo}`, null, null)
                }
            }
            if (correo[0].id_tipo_correo != req.body.id_tipo_correo) {
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 28, `Se modifica tipo correo ${correo[0].id_tipo_correo} por tipo correo ${req.body.id_tipo_correo}`, null, null)
            }
    ...
    async eliminar(req, res, next) {
        try {
            ...
            if (seBorro) {
                ...
                serviceTransaccion.generaTransaccion(req.headers.token, req.params.id, 26, `Se elimina correo ${req.params.correo}`, null, null)
            } else {
